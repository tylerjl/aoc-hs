...
mapRoute = toRoute North . map (toPath . filter (not . (==) ',')) . words
  where
    toRoute orientation ((lr, distance):xs) =
      replicate distance orientation' ++ toRoute orientation' xs
      where
        orientation' = turn lr orientation
    toRoute _ [] = []
